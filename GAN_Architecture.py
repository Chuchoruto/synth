...
class Discriminator(nn.Module):
    def __init__(self, input_dim, hidden_dims):
        ...
        for i in range(len(hidden_dims)):
            in_dim = input_dim if i == 0 else hidden_dims[i - 1]
            out_dim = hidden_dims[i]
            layers.append(nn.utils.parametrizations.spectral_norm(nn.Linear(in_dim, out_dim)))
            layers.append(nn.LeakyReLU(0.2, inplace=True))
...
# Create the architecture for the GAN based on data dimensions
def Create_GAN_Architecture(data_dim):
    generator_hidden_dim = min(1024, 10 * math.ceil(math.log(data_dim)))
    discriminator_hidden_dim = min(512, 5 * math.ceil(math.log(data_dim)))
    num_layers = min(4, math.ceil(math.log(data_dim)))
    ...
    generator = Generator(data_dim, data_dim, generator_hidden_dims)
    discriminator = Discriminator(data_dim, discriminator_hidden_dims)

    return generator, discriminator

# Calculate the gradient penalty for Wasserstein GAN with gradient penalty
...
# Training function for GAN with feature matching
def train_GAN_with_feature_matching(dataset, num_epochs=5000, batch_size=128, lr=0.00001, beta1=0.5, critic_updates=5, mse_weight=100, feature_weight=3000, noise_std=0.1):
    ...
    data_dim = dataset_np.shape[1]  # Use the full dimensionality including embeddings

    generator, discriminator = Create_GAN_Architecture(data_dim)
    ...
    for epoch in range(num_epochs):
        for real_data in dataloader:
            ...
            for _ in range(critic_updates):
                ...
                z = torch.randn(current_batch_size, data_dim).to(generator.model[0].weight.device)
...
# Standard GAN training function without feature matching
def train_GAN(dataset, num_epochs=5000, batch_size=128, lr=0.00001, beta1=0.5, critic_updates=5, mse_weight=10):
    dataset_np = dataset.to_numpy()
    data_dim = dataset_np.shape[1]  # Adjust for new data dimension with embeddings
